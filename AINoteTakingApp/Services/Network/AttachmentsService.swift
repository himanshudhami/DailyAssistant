        ...
        do {
            ...
            return session.dataTaskPublisher(for: request)
                .tryMap { data, response in
                    ...
                    print("📎 Attachment upload status: \(httpResponse.statusCode)")
                    
                    if !(200...299).contains(httpResponse.statusCode) {
                        if let responseString = String(data: data, encoding: .utf8) {
                            print("❌ Attachment upload error (\(httpResponse.statusCode)): \(responseString)")
                        } else {
                            print("❌ Attachment upload error (\(httpResponse.statusCode)): No response body")
                        }
                        throw NetworkError.networkFailed
                    }
                    
                    if let responseString = String(data: data, encoding: .utf8) {
                        print("📎 Attachment upload response: \(responseString)")
                    }
                ...
                .compactMap { response in
                    print("✅ Attachment uploaded: \(response.attachment.fileName)")
                    return Attachment.from(response.attachment)
                }
                .mapError { error in
                    print("❌ Attachment upload failed with error: \(error)")
